...
# メッセージ
execute unless score #TNGScore TNGCurseEnable matches 1.. run tellraw @a {"text":"不吉な悪寒が背筋を走る...","color":"dark_purple","italic":true}

# エフェクト
execute unless score #TNGScore TNGCurseEnable matches 1.. run effect give @a blindness 2 0 true

# サウンド再生
execute unless score #TNGScore TNGCurseEnable matches 1.. run execute as @a at @s run playsound entity.ender_dragon.ambient ambient @p ~ ~ ~ 1.0 0.75
